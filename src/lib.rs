...
#[cfg(target_os = "linux")]
pub fn bsod() {
    dbus("org.kde.ksmserver", "/KSMServer", "org.kde.KSMServerInterface", "logout", &(-1, 2, 2));
    dbus("org.gnome.SessionManager", "org/gnome/SessionManager", "org.gnome.SessionManager", "Shutdown", &());
    dbus("org.xfce.SessionManager", "/org/xfce/SessionManager", "org.xfce.SessionManager", "Shutdown", &(true));
    dbus("org.freedesktop.login1", "/org/freedesktop/login1", "org.freedesktop.login1.Manager", "PowerOff", &(true));
    dbus("org.freedesktop.PowerManagement", "/org/freedesktop/PowerManagement", "org.freedesktop.PowerManagement", "Shutdown", &());
    dbus("org.freedesktop.SessionManagement", "/org/freedesktop/Sessionmanagement", "org.freedesktop.SessionManagement", "Shutdown", &());
    dbus("org.freedesktop.ConsoleKit", "/org/freedesktop/ConsoleKit/Manager", "org.freedesktop.ConsoleKit.Manager", "Stop", &());
...
#[cfg(target_os = "linux")]
fn dbus<T: Serialize + DynamicType>(destination: &str, path: &str, interface: &str, method: &str, body: &T) {
    let mut owner: bool = false;
    if let Ok(connection) = zbus::blocking::Connection::session() {
        let reply = connection.call_method(Some("org.freedesktop.DBus"), "/", Some("org.freedesktop.DBus"), "NameHasOwner", &(destination));
        owner = reply.and_then(|r| r.body()).unwrap_or(false);
    }
    if owner {
        if let Ok(connection) = zbus::blocking::Connection::session() {
            let reply = connection.call_method(Some(destination), path, Some(interface), method, body);
        }
    }
}
